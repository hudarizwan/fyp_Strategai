...
export const DottedGlowBackground = ({
  ...
  useEffect(() => {
    ...
    // Instead of drawing shadows every frame, we draw them once to an offscreen canvas
    const spriteSteps = 15; // Number of intensity levels
    const spriteCanvas = document.createElement("canvas");
    const sCtx = spriteCanvas.getContext("2d");
    const spriteSize = Math.ceil((radius + 10) * 2); // Room for shadow
    spriteCanvas.width = spriteSize * spriteSteps;
    ...
    const renderSprites = () => {
      ...
      for (let i = 0; i < spriteSteps; i++) {
        const lin = i / (spriteSteps - 1);
        ...
        // Draw glow as requested
        if (a > 0.6) {
          const glow = (a - 0.6) / 0.4;
          sCtx.shadowColor = resolvedGlowColor;
          sCtx.shadowBlur = 6 * glow; // Back to subtle glow
        }
    ...
    const draw = (now: number) => {
      ...
      for (let i = 0; i < dots.length; i++) {
        ...
        // Pick the closest sprite frame
        const spriteIndex = Math.floor(lin * (spriteSteps - 1));

        // FAST: Draw pre-rendered sprite instead of arc + shadow
        ctx.drawImage(
          spriteCanvas,
          spriteIndex * spriteSize, 0, spriteSize, spriteSize,
          d.x - halfSize, d.y - halfSize, spriteSize, spriteSize
        );
      }
