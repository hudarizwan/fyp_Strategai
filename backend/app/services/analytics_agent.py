...
class AnalyticsAgent:
    ...
    def _build_recommendation_response(
        ...
        gross_margin = float(profitability_summary.get("gross_margin_percent", pricing["expected_margin_percent"]))
        gross_profit = float(profitability_summary.get("gross_profit_pkr", max(pricing["recommended_sell"] - pricing["recommended_buy"], 0.0)))
        observed_market_spread = float(profitability_summary.get("observed_market_spread_pkr", gross_profit))
        ...
        return {
            ...
            "expected_profit_margin": gross_margin,
            "confidence_score": primary.confidence_score,
    ...
    def _build_reasoning_bullets(self, primary: ClusterRecommendation, low_sample_warning: bool) -> List[str]:
        ...
        gross_margin = profitability_summary.get("gross_margin_percent")
        observed_spread = profitability_summary.get("observed_market_spread_pkr")
        if gross_margin is not None:
            bullets.append(
                f"Gross margin based on the observed market spread is {float(gross_margin):.1f}%."
            )
        if observed_spread is not None:
            bullets.append(f"Observed market spread sits around PKR {float(observed_spread):,.0f}.")
    ...
    def _generate_reasoning_summary(
        ...
        if commercial_intelligence:
            ...
            if profitability:
                parts.append(
                    f"Gross margin from the observed market spread is {profitability.get('gross_margin_percent', 0.0):.2f}% with observed spread near PKR {profitability.get('observed_market_spread_pkr', 0.0):.0f}."
                )
    ...
    def _build_commercial_intelligence(
        ...
        *,
        ...
        return {
            ...
            "market_intelligence": market_intelligence,
            "evidence_ledger": evidence_ledger,
            "net_margin_percent": profitability.get("gross_margin_percent", 0.0),
